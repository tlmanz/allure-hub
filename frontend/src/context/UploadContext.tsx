...
interface UploadContextValue {
  ...
  openDrawer: () => void
  closeDrawer: () => void
  startUpload: (envId: string, projectId: string, buildId: string, file: File, reportConfig?: Record<string, unknown>) => void
...
export function UploadProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    api.listUploadSessions().then(list => setSessions(list)).catch(() => {})
  ...
  const startUpload = useCallback(
    (envId: string, projectId: string, buildId: string, file: File, reportConfig?: Record<string, unknown>) => {
      setDrawerOpen(true)
      ...
      api.uploadResults(envId, projectId, buildId, file, () => {}, controller.signal, reportConfig)
        .catch((err: unknown) => {
  ...
  const retryUpload = useCallback((session: UploadSession) => {
    ...
    if (failedAtPhase === 'assembling') {
      api.retryAssembly(envId, projectId, uploadId)
        .then(runGenerate)
        .catch((err: unknown) => {
...
export function useActiveUploadCount(): number {
  ...
  return sessions.filter(s => s.phase === 'uploading' || s.phase === 'assembling' || s.phase === 'generating').length
}

...
export const api = {
  ...
  listProjects: (envId: string, signal?: AbortSignal) =>
    request<Project[]>(`/environments/${enc(envId)}/projects`, { signal }).then(
      (d) => d ?? [],
    ),
  ...
  deleteProject: (envId: string, id: string) =>
    request<void>(`/environments/${enc(envId)}/projects/${enc(id)}`, {
      method: "DELETE",
    }),
  ...
  listReportsPaged: (
    ...
    request<PagedReports>(
      `/environments/${enc(envId)}/projects/${enc(projectId)}/reports?limit=${limit}&offset=${offset}&filter=${enc(filter)}`,
      { signal },
    ),

  getReportStats: (envId: string, projectId: string, signal?: AbortSignal) =>
    request<ReportStats>(
      `/environments/${enc(envId)}/projects/${enc(projectId)}/reports/stats`,
      { signal },
    ),

  deleteReport: (envId: string, projectId: string, buildId: string) =>
    request<void>(
      `/environments/${enc(envId)}/projects/${enc(projectId)}/reports/${enc(buildId)}`,
      { method: "DELETE" },
    ),
  ...
  listNotifications: (limit = 50) =>
    request<NotificationItem[]>(`/notifications?limit=${limit}`).then(
      (d) => d ?? [],
    ),

  getUnreadNotificationCount: () =>
    request<{ count: number }>("/notifications/unread").then(
      (d) => d?.count ?? 0,
    ),
  ...
  deleteAPIKey: (id: string) =>
    request<void>(`/settings/apikeys/${enc(id)}?action=delete`, {
      method: "DELETE",
    }),
  ...
  listUsers: (search = "", offset = 0) =>
    request<PagedUsers>(
      `/settings/users?search=${enc(search)}&offset=${offset}`,
    ),
  ...
  getCleanupRuns: (limit = 5) =>
    request<CleanupRun[]>(`/settings/retention/runs?limit=${limit}`).then(
      (d) => d ?? [],
    ),
  ...
  // Settings - disk usage
  getDiskUsage: () => request<DiskUsage>("/settings/disk"),

  getDiskNotificationThreshold: () =>
    request<{ thresholdPercent: number }>(
      "/settings/disk/notification-threshold",
    ),

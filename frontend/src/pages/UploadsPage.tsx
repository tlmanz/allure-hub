...
function isActive(phase: UploadPhase) {
  return (
    phase === "uploading" || phase === "assembling" || phase === "generating"
  );
}
      ...
      <div className="flex items-center gap-5 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              className={`text-[10px] font-label font-bold uppercase tracking-wider px-2 py-0.5 rounded-full shrink-0 ${ph.badge}`}
            >
              {ph.label}
            </span>
          </div>
            ...
            {hasDoneWarning && (
              <>
                ...
                <button
                  ...
                  title={
                    warningExpanded ? "Collapse warnings" : "Expand warnings"
                  }
                >
        ...
        {s.phase !== "failed" && (
          <div className="w-36 shrink-0">
            <div className="flex justify-between text-[10px] text-on-surface-variant font-label mb-1">
              <span>{ph.label}</span>
              <span>
                {s.phase === "uploading" && pct === 0 ? "…" : `${pct}%`}
              </span>
            </div>
            <div className="h-1.5 bg-surface-container-highest rounded-full overflow-hidden">
              {s.phase === "assembling" ||
              s.phase === "generating" ||
              (s.phase === "uploading" && pct === 0) ? (
...
export default function UploadsPage() {
  ...
  const { can } = useAuth();
  const { pending, deleting, requestDelete, cancel, confirm } =
    useDeleteSession();
  ...
  const filtered = useMemo(() => {
    switch (filter) {
      case "active":
        return sessions.filter((s) => isActive(s.phase));
      case "done":
        return sessions.filter((s) => s.phase === "done");
      case "failed":
        return sessions.filter((s) => s.phase === "failed");
      default:
        return sessions;
    }
  }, [sessions, filter]);
  ...
  const FILTERS: { key: Filter; label: string; activeCls: string }[] = [
    {
      key: "all",
      label: "All",
      activeCls: "bg-surface-container-highest text-on-surface",
    },
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
          {[
            { label: "Total", value: counts.all, color: "text-on-surface" },
            {
              label: "Active",
              value: counts.active,
              color: counts.active > 0 ? "text-primary" : "text-on-surface",
            },
            {
              label: "Done",
              value: counts.done,
              color: counts.done > 0 ? successText : "text-on-surface",
            },
            {
              label: "Failed",
              value: counts.failed,
              color: counts.failed > 0 ? "text-error" : "text-on-surface",
            },
            ...
            >
              ...
              <p className={`text-3xl font-headline font-bold ${color}`}>
                {value}
              </p>
            </div>
          ))}
